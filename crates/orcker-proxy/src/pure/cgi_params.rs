...
//!
//! `SERVER_SOFTWARE` is deliberately `"orcker (nginx-compatible)"`, not just
//! `"orcker"`: frameworks (WordPress in particular - see `got_url_rewrite()` /
...
//! isn't universal. orcker's front-controller resolution
...
//! `Server:` HTTP header ([`orcker_core::PROXY_SERVER_ID`]), which still
//! identifies orcker honestly to browsers and tools.
...
/// `ORCKER_LOGIN_USER` param carrying the target username - see
/// [`AutoLoginParams`].
#[must_use]
...
pub fn build_params(
    ...
    push(&mut out, b"SERVER_SOFTWARE", b"orcker (nginx-compatible)");
    if https {
    ...
    if let Some(login) = auto_login {
        ...
        push(&mut out, b"ORCKER_LOGIN_USER", login.target_user.as_bytes());
    }
...
mod tests {
    ...
    fn web_root_subdir_drives_script_filename_and_document_root() {
        let mut site =
            orcker_core::Site::linked("app", "/srv/www/app", orcker_core::PhpVersion::new(8, 3))
                .unwrap();
    ...
    fn auto_login_adds_prepend_and_target_user_params() {
        ...
        assert_eq!(
            lookup(&pairs, b"ORCKER_LOGIN_USER"),
    ...
    fn auto_login_with_no_preference_sends_empty_target_user() {
        ...
        assert_eq!(lookup(&pairs, b"ORCKER_LOGIN_USER"), Some(b"".as_slice()));
    }
    ...
    fn no_auto_login_omits_prepend_and_target_user_params() {
        ...
        assert!(lookup(&pairs, b"ORCKER_LOGIN_USER").is_none());
    }
