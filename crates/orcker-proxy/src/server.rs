...
use orcker_core::{match_rule, Route, RouteRule, Site, SiteKind, SiteRouter, UpstreamTarget};
...
/// [`orcker_core::Site`]; the daemon swaps the whole router under a write guard
/// when a site is parked/linked/unlinked or its PHP version changes.
pub type SharedRouter = Arc<tokio::sync::RwLock<orcker_core::SiteRouter>>;
...
pub struct HttpsBinding<C: CertStore> {
    ...
    /// bind 8443 while a live privileged-port redirect, e.g. `orcker elevate
...
impl ProxyServer {
    ...
    /// **both** the HTTP and HTTPS accept loops by [`orcker_core::is_lan_source`]
            ...
            loop {
                tokio::select! {
                    ...
                    accepted = http_listener.accept() => {
                        match accepted {
                            Ok((stream, peer)) => {
                                if peer_filter && !orcker_core::is_lan_source(peer.ip()) {
                                    continue;
                                }
...
/// precedence only holds for GET/HEAD, the sole methods orcker serves static
...
fn warn_unusable_target(rule: &RouteRule, requested_path: &str) {
    tracing::warn!(
        target: "orcker_proxy::route_rule",
        prefix = %rule.prefix(),
...
/// (adding `auto_prepend_file`/`ORCKER_LOGIN_USER`).
...
/// or the site is switched to front-controller mode - Orcker has no way to
...
enum UnboundDecision {
    /// Serve this site directly - a pin-cookie hit or an `X-Orcker-Site` header.
    Serve(Site),
...
///
/// Priority: explicit `X-Orcker-Site` header → bare `/~` clear → `/~domain`
...
/// Read the per-request site directive header (`X-Orcker-Site`, or its dash-free
...
/// `Server: orcker` - stamped on every synthetic (proxy-originated) response so
/// the macOS port-redirect probe can confirm a connection to 80/443 actually
/// reaches *this* proxy, not some other listener. See [`orcker_core::PROXY_SERVER_ID`].
...
mod unbound_tests {
    use super::*;
    use orcker_core::{PhpVersion, RouterConfig};
    ...
    fn switch_beats_existing_cookie() {
        assert_eq!(
            match classify("/~blog.test/p", None, None, Some("orcker-site=app"), HTML) {
    ...
    fn bare_tilde_clears_pin() {
        for path in ["/~", "/~/"] {
            ...
            assert!(
                matches!(
                    classify(path, None, None, Some("orcker-site=app"), HTML),
                    UnboundDecision::ClearPin
                ),
    ...
    fn stale_cookie_clears_and_picks_or_404s() {
        match classify("/x", None, None, Some("orcker-site=ghost"), HTML) {
            UnboundDecision::Picker { dest, clear } => {
                assert_eq!(dest, "/x");
                assert!(clear);
            }
    ...
    fn site_header_value_prefers_canonical_then_alias() {
        ...
        h.insert("x-orckersite", "blog".parse().unwrap());
        assert_eq!(site_header_value(&h), Some("blog"));
        h.insert("x-orcker-site", "app.test".parse().unwrap());
    ...
    fn clear_response_shape() {
        ...
        assert!(set_cookie.contains("orcker-site=;"));
    }
...
mod login_token_tests {
    ...
    fn valid_token_strips_query_and_returns_target_user() {
        let mut req = get_req("/wp-admin/?a=1&orcker_login_token=abc&b=2");
    ...
    fn valid_token_with_configured_user_returns_that_username() {
        let mut req = get_req("/wp-admin/?orcker_login_token=abc");
        let consumer = FakeConsumer {
            valid: true,
            target_user: "editor",
        };
    ...
    fn path_outside_wp_admin_is_never_considered() {
        let mut req = get_req("/?orcker_login_token=abc");
        let consumer = FakeConsumer {
            valid: true,
            target_user: "",
        };
        assert!(consume_login_token_if_present(&mut req, "blog", &consumer).is_none());
        assert_eq!(path_and_query(&req), "/?orcker_login_token=abc");
    }
    ...
    fn invalid_token_leaves_query_untouched() {
        ...
        assert!(consume_login_token_if_present(&mut req, "blog", &consumer).is_none());
        assert_eq!(path_and_query(&req), "/wp-admin/?orcker_login_token=abc");
    }
...
mod classify_fallthrough_tests {
    use super::*;
    use orcker_core::{PhpVersion, RouterConfig, Site};
    ...
    /// A cookie that carries no `orcker-site` pin must NOT clear anything and
