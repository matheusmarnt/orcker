...
use orcker_ipc::{BrowserTrust, Diagnosis, DiagnosisCode, Severity, StatusReport};
...
pub fn diagnose(report: &StatusReport, path_needs_setup: Option<bool>) -> Vec<Diagnosis> {
        ...
        .any(|d| matches!(d.severity, Severity::Warn | Severity::Fail))
    {
        out.push(Diagnosis {
            ...
            detail: "Daemon, ports, DNS, and CA look healthy.".to_owned(),
            remedy: None,
        });
    }
...
///
/// Empty for the whole of phase 0: the two auto-fixable findings this fork
/// inherited (restart a failed FPM pool, rebuild the PHP CA bundle) both died
/// with the native runtime. Docker-side safe fixes arrive with PRD FR-071;
/// privileged or slow remediation stays the user's to run either way.
#[must_use]
pub fn plan_auto_fixes(_report: &StatusReport) -> Vec<FixAction> {
    Vec::new()
}
...
mod tests {
    use super::*;
    use orcker_ipc::{CaStatus, PortRedirectTargets, PortStatus, SiteCounts, StatusReport};
