---
applyTo: "bin/orcker/**/*.rs"
---

# orcker — the CLI

A `orcker-ipc` **client** of the daemon. It maps commands to IPC requests, renders
...
- The IPC client transport wiring (`transport.rs`) using `orcker-ipc`'s
  `transport` feature.
- `orcker elevate` (`elevate.rs`): when run under sudo, it owns the
  `Command::new(...)` that invokes `orcker-helper` with a typed `HelperInvocation`
  from `orcker-platform`.
...
- Reimplement anything the daemon owns (routing, supervision, config authority).
  If the CLI needs a new capability, add a `Request`/`Response` to `orcker-ipc`
  and let the daemon implement it.
- Perform privileged effects inline beyond owning the explicit `orcker elevate`
  helper invocation.
