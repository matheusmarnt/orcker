...
pub async fn start_log_stream(
    ...
    // Auto-detect app container — optional: docker stream skipped if no container found
    let app_container =
        crate::adapters::docker::containers::find_app_container(&docker, &project_path).await;

    if let Some(container) = app_container {
    ...
    // laravel.log file tail — runs regardless of container state
