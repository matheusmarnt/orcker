#!/bin/sh
# Orcker GUI .deb post-install.
#
# Tauri's deb bundler copies the GUI **and** its externalBin sidecars (orcker,
# orckerd, orcker-helper) side-by-side into /usr/bin — so they are already on PATH
# and already siblings of /usr/bin/orcker-gui. No symlinking is needed in that
...
if command -v setcap >/dev/null 2>&1; then
  ...
  echo "orcker: setcap not found (install libcap2-bin); the daemon will use 8080/8443" >&2
fi
